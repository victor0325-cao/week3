...
def create_async_mysql_enginc(conf, is_slave=False):
    engine = create_async_engine(
        ...
        pool_recycle = 3600,             #定义连接池的连接回收时间
        pool_pre_ping = True             #启用或禁用连接池中的连接在使用自检测功能，True表示启用
    )
...
#在这个上下文管理器中，会提供的异步会话类创建一个会话实例，提供一个会话对象供调用方执行数据库操作。
#如果设置了commit=True，则在with块结束时会自动提交事务；否则，会回滚事务。
#最后，无论如何，都会关闭会话
@asynccontextmanager
